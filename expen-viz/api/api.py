...
def home():
    ...
    pie_categorical_sum_expenditure = pieCategoricalSumExpenditure(
        data, from_date, to_date)
    graphs = (scatter_bar_daily_sum_and_average, pie_days_expenditure_pie_chart,
              bar_day_based_categorical_expenditure, bar_categorical_sum_expenditure, pie_categorical_sum_expenditure)
...
def createDataFrame(final_csv):
    ...
    data.columns = ["date", "journal", "action", "to", "category",
                    "item", "debit", "credit", "balance"]  # set the column headers
...
def scatterbarDailySumAndAverage(data, from_date, to_date):
    ...
    layout = go.Layout(
        ...
        annotations=[go.layout.Annotation(text='* No average = sum', align='left', showarrow=False, xref='paper', yref='paper', y=1.0291, x=1, width=400, bordercolor='gray', borderwidth=1, opacity=0.5)])
...
def pieDaysExpenditurePieChart(data, from_date, to_date):
    ...
    df = df.loc[(df["category"] != "UNKNOWN") & (df["category"] != "FAMILY") & (df["category"] != "RENT") & (df["category"] != "SALARY") & (df["category"] != "CREDIT") & (
        df["category"] != "COMPUTER MONITOR") & (df["category"] != "FURNITURE") & (df["category"] != "RETURN") & (df["category"] != "HALF YR.ANNUAL MAINT CHR") & (df["category"] != "INTEREST")]
...
def barDayBasedCategoricalExpenditure(data, from_date, to_date):
    ...
    fig = px.bar(df, x="category", y="debit", barmode="group", facet_col="day",
                 text="debit", title="Spending based on Days of the week by categories")
    # fig.update_layout(paper_bgcolor="rgba(0,0,0,0)")
    graphJSON = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
    return graphJSON
...
def barCategoricalSumExpenditure(data, from_date, to_date):
    ...
    start_date, end_date = get_dates(from_date, to_date)
    df = data.loc[(data['date'] > start_date) & (data['date'] <= end_date)]
    fig = px.bar(df, x="category", y="debit", labels={'debit': 'Expenditure'}, hover_data=[
                 'category', 'item', 'date', 'debit', 'balance'])
    # fig.update_layout(paper_bgcolor="rgba(0,0,0,0)")
    graphJSON = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
    return graphJSON
...
def scatterDailyExpenditure(data, from_date, to_date):
    ...
    start_date, end_date = get_dates(from_date, to_date)
    df = data.loc[(data['date'] > start_date) & (data['date'] <= end_date)]
    fig = px.scatter(df, x="date", y="debit", labels={'debit': 'Expenditure'}, hover_data=[
                     'category', 'item', 'date', 'debit', 'balance'])
    # fig.update_layout(paper_bgcolor="rgba(0,0,0,0)")
    graphJSON = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
    return graphJSON
